...
The presentation layer the outermost layer responsible processing the user's requests, calling to the application layer to perform the operations related to the request, and serving the responses. The input DTOs to this layer are called **requests** and the output DTOs are called **responses**. Requests represent data sent by a user to one of this projects APIs or services. Responses represent the data sent back to those users. The presentation layer makes call to services in the application layer to perform the operation requested by the user.

### 2. The Application Layer

The application layer is responsible for orchestrating application logic i.e. the sequencing of tasks needed to fulfill a user's request. This layer is broken up into three parts: Services, ports, and input/output DTOs. First, the presentation layer calls into the application layer by invoking a service's method with user's validated request data (inputs). The service then executes the sequences of tasks that fulfill that request. Some such tasks include calls to databases and message queues (the infrastructure layer). Invocation of the infrastructure happens through repositories (to access databases) and event handlers (to send messages to brokers). These infrastructure components are furnished to the service via dependency injection (DI) in the bootstrap layer (discussed later). The concrete implementations of these repositories and event handlers are not known to the service directly, but known only through contracts called "ports". Traits are used to describe the contracts that these concrete implementations will abide by. The application layer only directly knows about the domain layer (discussed later) which is a pure representation of the business space, its entities, and the rules that govern these entities. These entities are the values returned from a service's methods. We call the entities returned from the methods "outputs". In pure DDD, we would first translate these entities into an application layer-specific struct and pass that back to the presentation layer, but in an effort to reduce the amount of code written, we are simply returning entities. This is indeed a leaky abstraction, but by removing this layer of indirection, we can improve the speed of development. Additionally, it is a relatively simple refactor when we want to implement DDD in a more complete way and the additional abstraction doesn't buy us much.
