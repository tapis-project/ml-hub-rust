pub(crate) mod files {
    ...
    pub async fn mkdir(
        ...
        let url = build_operation_url(
            tenant,
            String::from("files"),
            Some(format!("{}/{}", String::from("ops"), system_id)),
        );

        let body = HashMap::new().insert("path", path);

        let request = client.post(url).form(&body);
        ...
        // Add token header
        request.headers(headers).send().await.map_err(|err| {
