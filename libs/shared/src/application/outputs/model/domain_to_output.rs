...
use crate::application::outputs::model as outputs;
use crate::domain::entities::deployment_strategy::strategy::Strategy;
use crate::domain::entities::model as entities;
use crate::errors::Error;

impl TryFrom<(&entities::Model, &Vec<Strategy>)> for outputs::Model {
    type Error = Error;

    fn try_from(value: (&entities::Model, &Vec<Strategy>)) -> Result<Self, Self::Error> {
        let (model, strategies) = value;

        let canonical = model.canonical.clone().map(|c| outputs::Canonical::from(c));

        // Hashmap for O(1) lookup
        let strategy_description_map: std::collections::HashMap<String, Option<String>> =
            strategies
                .iter()
                .map(|s| {
                    (
                        format_strategy_description_key(&s.platform, &s.name),
                        s.description.clone(),
                    )
                })
                .collect();

        let deployment_strategy_refs: Vec<outputs::DeploymentStrategyReference> = model
