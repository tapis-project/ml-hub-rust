#[cfg(test)]
mod endpoint_query_service_test {
    ...
    use crate::application::services::endpoint_query_service::{
        EndpointQueryService, EndpointQueryServiceError,
    };
    ...
    ) -> Result<(), EndpointQueryServiceError> {
        let repository = Arc::new(RecordingEndpointRepository::new());
        let service = EndpointQueryService::new(repository.clone());
    ...
    async fn propagates_repository_errors() {
        let service = EndpointQueryService::new(Arc::new(FailingEndpointRepository));
        ...
        assert!(matches!(
            result,
            Err(EndpointQueryServiceError::Repository(_))
        ));
    }
}
