use crate::application::inputs::principal::{
    FindByFederatedIdentity, GetOrCreateFromFederatedIdentity,
};
...
use retry_utils::{retry_async, FixedBackoff, Retry, RetryPolicy, RetryStrategyAction};
use thiserror::Error;
...
impl PrincipalService {
    ...
    pub async fn get_or_create_from_identity(
        &self,
        input: GetOrCreateFromFederatedIdentity,
    ) -> Result<Principal, PrincipalServiceError> {
        // Input for the repository call
        let find_by_identity_input = FindByFederatedIdentity {
            identity: input.identity.clone(),
        };

        // The repo call to be passed into the retrier
        let find_by_identity_op = || {
            self.principal_repository
                .find_by_identity(&find_by_identity_input)
        };
        ...
        let retry_strategy = |err: &PrincipalRepositoryError, _attempt| -> RetryStrategyAction {
            match err {
                ...
                | PrincipalRepositoryError::ProgrammingError(_) => {
        ...
        let maybe_principal = retry_async(
            find_by_identity_op,
...
impl From<PrincipalError> for PrincipalServiceError {
    fn from(value: PrincipalError) -> Self {
        match value {
            PrincipalError::TenantMismatch => {
                PrincipalServiceError::InternalError(value.to_string())
            }
        }
    }
}
...
impl From<PrincipalRepositoryError> for PrincipalServiceError {
    fn from(value: PrincipalRepositoryError) -> Self {
        match value {
            ...
            | PrincipalRepositoryError::PersistenceError { message: msg, .. } => {
                PrincipalServiceError::InternalError(msg)
            }
            PrincipalRepositoryError::FederatedIdentityAlreadyOwned(sub, iss) => {
                PrincipalServiceError::FederatedIdentityConflict(iss, sub)
            }
            PrincipalRepositoryError::PrincipalAlreadyExists => {
