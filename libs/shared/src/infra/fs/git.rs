...
impl GitRepository {
    ...
    fn build_remote_url(base_url: String, name: String) -> String {
        format!("{}/{}{}", base_url.clone(), name.clone(), ".git")
    }
    ...
    pub fn prepare(&self, params: PrepareRepositoryParams) -> Result<PreparedRepository, GitError> {
        ...
        create_dir_all(&target_path).map_err(|err| {
            GlobalLogger::error(
                format!(
                    "Error creating dirs '{:?}': {}",
                    &target_path,
            ...
            GitError::SystemError(SystemError::FileSystemError(format!(
                "{}: {:?}",
                err.to_string(),
                &target_path
            )))
...
impl PreparedRepository {
    ...
    fn clone(&self, params: GitCloneParams) -> Result<&Self, GitError> {
        ...
        cmd.arg(self.repository.remote_url.clone()).arg(".");
        ...
        match output.status.code() {
            ...
            None => {
                return Err(GitError::Clone(
                    "The git clone operation was terminated by an unknown signal".into(),
                ))
            }
        }
    }
...
impl GitLfsRepository {
    ...
    pub fn pull(&self, params: GitLfsPullLargeFilesParams) -> Result<&Self, GitError> {
        let output = Command::new("git")
            ...
            .map_err(|err| {
                GlobalLogger::debug(
                    format!("Error running `git lfs ls-files`: {}", err.to_string()).as_str(),
                );
        ...
        GlobalLogger::debug(
            format!("Files to include: {:#?}", &included_large_files_args).as_str(),
        );
        ...
        GlobalLogger::debug(
            format!("Files to exclude: {:#?} ", &excluded_large_files_args).as_str(),
        );
        ...
        cmd.output().map_err(|err| {
            GlobalLogger::error(
                format!("Error running `git lfs pull`: {}", err.to_string()).as_str(),
            );
...
pub trait SyncGitRepositoryImpl {
    fn sync_git_repo(
        &self,
        params: SyncGitRepositoryParams,
    ) -> Result<PreparedRepository, GitError>;
...
impl<T: SyncGitRepository> SyncGitRepositoryImpl for T {
    fn sync_git_repo(
        ...
        let repo = GitRepository::new(params.remote_base_url, params.name);

        let prepared_repo = repo.prepare(PrepareRepositoryParams {
    ...
    fn sync_lfs_repo(&self, params: SyncLfsRepositoryParams) -> Result<GitLfsRepository, GitError> {
        ...
        let git_lfs_repo = GitLfsRepository::from_prepared_git_repo(prepared_repo)?;
