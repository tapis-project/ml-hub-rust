// TODO Refactor: Should not be a requests. Needs mappings through to the
// infra layer
use crate::presentation::http::v1::requests::common::archive::Compression;
...
pub enum CompressionError {
    ...
    #[error("Error zipping/unzipping path: {0}")]
    ZipError(String),
}
...
impl Archiver {
    ...
    fn zip_dir(
        ...
        writer
            .add_directory_from_path(dir_path, options)
            .map_err(|err| CompressionError::ZipError(err.to_string()))?;

        let entries =
            fs::read_dir(path).map_err(|err| CompressionError::IOError(err.to_string()))?;

        for maybe_entry in entries {
            let entry = maybe_entry.map_err(|err| CompressionError::IOError(err.to_string()))?;
    ...
    pub fn unzip(
        ...
        let file = File::open(source).map_err(|e| CompressionError::IOError(e.to_string()))?;
    ...
    fn unzip_file<R: Read + Seek>(
        ...
        let mut archive =
            ZipArchive::new(file).map_err(|e| CompressionError::ZipError(e.to_string()))?;
    ...
    fn extract_entry<R: Read + Seek>(
        ...
        match () {
            // ──────────────── 1) dir ────────────────
            _ if name.ends_with('/') => {
                fs::create_dir_all(&out_path).map_err(|e| CompressionError::IOError(e.to_string()))
            }
