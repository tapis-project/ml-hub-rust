...
use crate::infra::artifacts::mongo::documents::{
    Artifact, ArtifactType, UpdateArtifactPathRequest, UpdateArtifactRequest,
};
...
impl application::ports::artifacts::ArtifactRepository for ArtifactRepository {
    async fn save(
        &self,
        artifact: &entities::artifact::Artifact,
    ...
    async fn list_by_artifact_type(
        &self,
        artifact_type: ArtifactTypeEntity,
    ) -> Result<Vec<entities::artifact::Artifact>, ArtifactRepositoryError> {
        let filter = doc! {
        ...
        while let Some(artifact) = cursor.try_next().await.map_err(|e| {
            let error = InfrastructureError::new_internal();
            log::error!(
                "[{}] Persistence error: {}",
                error.error_id(),
            ...
            artifacts.push(
                entities::artifact::Artifact::try_from(artifact).map_err(|e| {
                    let error = InfrastructureError::new_internal();
    ...
    async fn get_by_id(
        &self,
        id: &uuid::Uuid,
    ) -> Result<Option<entities::artifact::Artifact>, ArtifactRepositoryError> {
        let filter = doc! {
        ...
        while let Some(artifact_doc) = cursor.try_next().await.map_err(|e| {
            let error = InfrastructureError::new_internal();
            log::error!(
                "[{}] Persistence error: {}",
                error.error_id(),
            ...
            let artifact = entities::artifact::Artifact::try_from(artifact_doc).map_err(|e| {
                let error = InfrastructureError::new_internal();
                log::error!("[{}] Conversion error: {}", error.error_id(), e.to_string());
                error
            })?;
    ...
    async fn update(
        ...
        let update = UpdateArtifactRequest::try_from(artifact.clone()).map_err(|e| {
            let error = InfrastructureError::new_internal();
            log::error!("[{}] Conversion error: {}", error.error_id(), e.to_string());
            error
        })?;
    ...
    async fn update_path(
        ...
        let update = UpdateArtifactPathRequest::try_from(artifact.clone()).map_err(|e| {
            let error = InfrastructureError::new_internal();
            log::error!(
                "[{}] Persistence error: {}",
                error.error_id(),
