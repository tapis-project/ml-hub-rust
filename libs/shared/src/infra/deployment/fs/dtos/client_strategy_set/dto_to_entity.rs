...
impl TryFrom<dtos::client_strategy_set::ClientStrategySet>
    for entities::client_strategy_set::ClientStrategySet
{
    ...
    fn try_from(value: dtos::client_strategy_set::ClientStrategySet) -> Result<Self, Self::Error> {
        ...
        for strat in value.strategies {
            ...
            match maybe_strat {
                ...
                Err(err) => {
                    return Err(
                        entities::client_strategy_set::ClientStrategySetError::ClientStrategyError(
                            err,
                        ),
                    )
                }
        ...
        let maybe_rule_sets: Option<Vec<entities::rule_set::RuleSet>> = match value.rule_sets {
            Some(rs) => {
                let rule_sets: Vec<entities::rule_set::RuleSet> = rs
                    .iter()
                    .map(|r| entities::rule_set::RuleSet::from(r.clone()))
                    .collect();
        ...
        let maybe_parameter_sets: Option<Vec<entities::parameter_set::ParameterSet>> =
            match value.parameter_sets {
                Some(ps) => {
                    let parameter_sets: Vec<entities::parameter_set::ParameterSet> = ps
                        .iter()
                        .map(|p| entities::parameter_set::ParameterSet::from(p.clone()))
                        .collect();
