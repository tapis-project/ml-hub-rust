...
pub const MODEL_DEPLOYMENT_RECONCILIATION_EXCHANGE: &'static str =
    "exchange.model_deployment.reconcile";
pub const DEAD_LETTER_EXCHANGE: &'static str = "exchange.dead_letter";

pub async fn declare_exchanges(
    channel: &Channel,
    exchanges: Vec<(&'static str, ExchangeType)>,
) -> Result<(), BrokerError> {
    for (exchange, exchange_type) in exchanges {
        let exchange_args =
            ExchangeDeclareArguments::new(exchange, exchange_type.to_string().as_str()).finish();

        info!(
            "Declaring exchange {} with type {}",
            exchange,
