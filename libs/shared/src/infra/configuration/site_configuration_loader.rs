...
impl SiteConfigurationLoader {
    pub fn new() -> Result<Self, SiteConfigurationLoaderError> {
        let config_path =
            std::env::var("SITE_CONFIG_PATH").unwrap_or(DEFAULT_SITE_CONFGIURATION_PATH.into());

        // Load the client strategy set from the file
        let contents =
            std::fs::read_to_string(std::path::PathBuf::from(config_path)).map_err(|e| {
                error!(
                    "Error reading contents of site configuration file {}",
                    e.to_string()
                );
                SiteConfigurationLoaderError::LoadingError(format!(
                    "Failed to read config file: {}",
                    e.to_string()
                ))
            })?;

        let config = serde_json::from_str::<SiteConfiguration>(&contents).map_err(|e| {
            error!(
                "Failed to deserialize site configuration: {}",
                e.to_string()
            );
