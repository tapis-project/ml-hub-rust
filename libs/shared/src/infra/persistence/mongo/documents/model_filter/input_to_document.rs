...
use crate::infra::persistence::mongo::documents::model_filter;
use crate::infra::persistence::mongo::documents::task as document_task;

use mongodb::bson::doc;

impl TryFrom<(&inputs::SearchCriterion, &Vec<String>)> for model_filter::ModelFilter {
    type Error = Error;
    fn try_from(value: (&inputs::SearchCriterion, &Vec<String>)) -> Result<Self, Self::Error> {
        ...
        for task_type in search_criteria
            .task_types
            .clone()
        ...
        let mut tenancy_selector = Some(doc! {
            "$in": tenant_ids
        });
