...
use crate::infra::persistence::mongo::documents::model as infra;
...
impl TryFrom<infra::Model> for domain::Model {
    type Error = Error;

    fn try_from(value: infra::Model) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            artifact_id: value
                .artifact_id
                .and_then(|id| Some(Uuid::from_bytes(id.bytes()))),
            author: value.author,
