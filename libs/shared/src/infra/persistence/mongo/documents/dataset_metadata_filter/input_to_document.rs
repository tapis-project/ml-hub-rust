use crate::infra::persistence::mongo::documents::dataset_metadata_filter;
use crate::application::inputs::discover_datasets as inputs;
use crate::infra::persistence::mongo::documents::task as document_task;
use crate::errors::Error;

impl TryFrom<inputs::SystemRequirement> for dataset_metadata_filter::SystemRequirement {
    type Error = Error;
...
impl TryFrom<inputs::Accelerator> for dataset_metadata_filter::Accelerator {
    type Error = Error;
    
    fn try_from(value: inputs::Accelerator) -> Result<Self, Self::Error> {
        let system_requirements: Option<Vec<dataset_metadata_filter::SystemRequirement>> = match value.system_requirements {
            Some(sr) => {
                let mut reqs: Vec<dataset_metadata_filter::SystemRequirement> = Vec::with_capacity(1);
                for requirement in sr {
                    reqs.push(dataset_metadata_filter::SystemRequirement::try_from(requirement)?);
                }
...
impl TryFrom<inputs::HardwareRequirements> for dataset_metadata_filter::HardwareRequirements {
    type Error = Error;
    
    fn try_from(value: inputs::HardwareRequirements) -> Result<Self, Self::Error> {
        let mut accelerators: Vec<dataset_metadata_filter::Accelerator> = Vec::with_capacity(1);
        for accelerator in value.accelerators.unwrap_or(Vec::with_capacity(0)) {
            accelerators.push(dataset_metadata_filter::Accelerator::try_from(accelerator)?);
        }
...
impl TryFrom<inputs::DatasetIO> for dataset_metadata_filter::DatasetIO {
    type Error = Error;
...
impl TryFrom<&inputs::SearchCriterion> for dataset_metadata_filter::DatasetMetadataFilter {
    type Error = Error;
    ...
    fn try_from(value: &inputs::SearchCriterion) -> Result<Self, Self::Error> {
        ...
        for input in value.dataset_inputs.clone().unwrap_or(Vec::with_capacity(0)) {
            dataset_inputs.push(dataset_metadata_filter::DatasetIO::try_from(input)?)
        }
        
        let mut dataset_outputs = Vec::with_capacity(1);
        for output in value.dataset_outputs.clone().unwrap_or(Vec::with_capacity(0)) {
            dataset_outputs.push(dataset_metadata_filter::DatasetIO::try_from(output)?)
        }

        let inference_hardware = value.inference_hardware.clone()
            .map(|hardware| dataset_metadata_filter::HardwareRequirements::try_from(hardware))
            .transpose()?;
