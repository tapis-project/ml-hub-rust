...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "Listed model artifacts",
            body = contracts::responses::ListModelArtifactResponse
        ),
        (
            status = 400,
...
async fn list_model_artifacts(_req: HttpRequest, data: web::Data<AppState>) -> impl Responder {
    ...
    let response_dtos: Vec<responses::Artifact> = artifacts
        .into_iter()
