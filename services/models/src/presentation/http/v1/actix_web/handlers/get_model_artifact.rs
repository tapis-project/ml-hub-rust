...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "Found model artifact",
            body = contracts::responses::GetModelArtifactResponse
        ),
        (
            status = 400,
...
async fn get_model_artifact(
    ...
    let input = GetModelArtifactInput { artifact_id };
        ...
        Err(err) => match err {
            ...
            _ => {
                return build_error_response(
                    500,
                    format!("Error fetching artifact {}", &artifact_id),
                )
            }
        },
    };
