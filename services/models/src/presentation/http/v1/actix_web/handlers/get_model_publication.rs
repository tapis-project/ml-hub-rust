...
#[utoipa::path(
    ...
    params(
        (
            ...
            Path,
            description = "The ID of the model publication"
        ),
    ),
    responses(
        (
            ...
            body = contracts::responses::GetModelPublicationResponse
        ),
        (
            status = 400,
...
async fn get_model_publication(
    ...
    let input = GetModelPublicationInput { publication_id };
        ...
        Err(err) => match err {
            ArtifactServiceError::IncorrectArtifactType(_) => {
                return build_error_response(
                    404,
                    format!(
                        "Model Artifact Publication not for id '{}'",
                        &publication_id
                    ),
                )
            }
    ...
    let publication = match maybe_publication {
        ...
        None => {
            return build_error_response(
                404,
                format!("ArtifactPublication with id {} not found", &publication_id),
            )
        }
    };
