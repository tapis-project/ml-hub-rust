...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "Listed model ingestions",
            body = contracts::responses::ListModelIngestionsResponse
        ),
        (
            status = 400,
...
async fn list_model_ingestions(_req: HttpRequest, data: web::Data<AppState>) -> impl Responder {
    ...
    let ingestions = match artifact_service
        .list_model_ingestions(ListModelIngestionsInput {})
        .await
    {
        Ok(a) => a,
    ...
    let response_dtos: Vec<responses::ArtifactIngestion> = ingestions
        .into_iter()
