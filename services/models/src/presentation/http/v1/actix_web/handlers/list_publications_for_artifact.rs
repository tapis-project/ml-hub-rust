...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "Listed model publications for artifact",
            body = contracts::responses::ListModelPublicationsForArtifactResponse
        ),
        (
            status = 400,
...
async fn list_publications_for_artifact(
    ...
    let input = ListPublicationsByArtifactIdInput { artifact_id };

    let publications = match artifact_service
        .list_publications_by_artifact_id(input)
        .await
    {
        Ok(p) => p,
