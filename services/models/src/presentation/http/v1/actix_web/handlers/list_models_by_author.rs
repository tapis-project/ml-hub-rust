...
use shared::application::inputs::model::ListModelsByAuthorInput;
...
async fn list_models_by_author(
    ...
    let input = ListModelsByAuthorInput {
    ...
    let output = match model_service.list_by_author(input).await {
        Ok(m) => m,
    ...
    for model in output.models {
        let model_resp = match Model::try_from(&model) {
            Ok(m) => m,
            Err(err) => return build_error_response(500, err.to_string())
        };
