...
#[utoipa::path(
    ...
    params(
        (
            "ingestion_id" = String,
            Path,
            description = "The ID of the model ingestion"
        ),
    ),
    responses(
        (
            status = 200,
            description = "Fetched Model Ingestion",
            body = contracts::responses::GetModelIngestionResponse
        ),
        (
            status = 400,
...
async fn get_model_ingestion(
    ...
    let input = GetModelIngestionInput { ingestion_id };

    let maybe_ingestion = match artifact_service.get_model_ingestion(input).await {
        Ok(a) => a,
        Err(err) => match err {
            ArtifactServiceError::IncorrectArtifactType(_) => {
                return build_error_response(
                    404,
                    format!("Model Artifact Ingestion not for id '{}'", &ingestion_id),
                )
            }
    ...
    let ingestion = match maybe_ingestion {
        ...
        None => {
            return build_error_response(
                404,
                format!("ArtifactIngestion with id {} not found", &ingestion_id),
            )
        }
    };
