...
use crate::presentation::http::v1::requests::{
    Headers, PublishArtifactPath, PublishArtifactRequest, PublishArtifactServiceRequest,
};
...
#[utoipa::path(
    ...
    params(
        (
            "artifact_id" = String,
            Path,
            description = "The ID of the model artifact"
        ),
    ),
    ...
    responses(
        (
            ...
            body = contracts::responses::PublishModelArtifactResponse
        ),
        (
            status = 400,
...
async fn publish_model_artifact(
    ...
    build_success_response(
        Some(resp),
        Some("Successfully submitted artifact publication".into()),
        None,
    )
}
