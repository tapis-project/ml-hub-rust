...
use crate::presentation::http::v1::requests::{Headers, ListDatasetsByPlatformPath, ListDatasetsByPlatformRequest};
use actix_web::{get, web, HttpRequest, Responder};
use client_provider::ClientProvider;
use clients::ListDatasetsClient;
...
#[utoipa::path(
    get,
    path="/datasets-api/platforms/{platform}/datasets",
    tag="Platforms",
    description="List datasets from an external platform",
    params(
        ("platform" = Platform, Path, description = "The platform for which you want to list the datasets"),
    ),
    responses(
        (status=200, description="Listed datasets", body=contracts::responses::ListDatasetsByPlatformResponse),
...
async fn list_datasets_by_platform(
    req: HttpRequest,
    path: web::Path<ListDatasetsByPlatformPath>,
    query: web::Query<HashMap<String, String>>,
    body: web::Bytes,
) -> impl Responder {
    let logger = SharedLogger::new();
    logger.debug("Start operation list_datasets");
    logger.debug(format!("path: {:#?}", path).as_str());

    // Get the client for the provided platform
    let client = if let Ok(client) = ClientProvider::provide_list_datasets_client(&path.platform) {
        client
    } else {
    ...
    let request = ListDatasetsByPlatformRequest {
        headers,
