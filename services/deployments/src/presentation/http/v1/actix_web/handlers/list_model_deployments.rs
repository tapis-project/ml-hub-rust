...
#[utoipa::path(
    ...
    responses(
        (
            status = 200,
            description = "A list of model deployments",
            body = contracts::responses::ListModelDeploymentsResponse
        ),
        (
            status = 400,
...
async fn list_model_deployments(
    ...
    let maybe_deployments = service
        .list_by_owner(
            ListModelDeploymentsByOwnerInput {
