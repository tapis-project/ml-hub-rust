    ...
    def run_smoke_test(self) -> Dict:
        ...
        response = requests.post(
            ...
            headers={
                ...
                "X-FlexServ-Secret": self.flexserv_secret,
            },
        ...
        try:
            ...
            logger.warning(
                "Smoke test response for %s was not JSON (status %s): %.200s",
                self.url,
                status,
                body_preview,
            )
            return {"status_code": status, "payload": None, "raw_body": body_preview}

        return {"status_code": status, "payload": data}
...
class DeploymentRegistry:
    ...
    def create(self, deployment_id: Optional[str] = None) -> DeploymentState:
        ...
        state = DeploymentState(
            deployment_id=deployment_id, message="Deployment queued"
        )
        with self._lock:
    ...
    def update(self, deployment_id: str, **updates: Any) -> DeploymentState:
        with self._lock:
            ...
            print(f"Deployment {deployment_id} updated: {updates}")
            # logger.debug("Deployment %s updated: %s", deployment_id, updates)
            return new_state
...
class FlexDeploymentRequest(BaseModel):
    ...
    model_cache_dir: Optional[Path] = (
        None  # place for holding downloaded model files. this directory is a copy of the model.
    )
...
class BaseFlexDeploymentWorkflow(ABC):
    ...
    def _run_workflow(
        self, deployment_id: str, request: FlexDeploymentRequest, api: HfApi
    ) -> None:
        try:
            ...
            self._start_monitoring_thread(
                deployment_id, request, model, dataset, handle
            )
    ...
    def _monitor_wrapper(
        ...
        try:
            result = self.monitor_deployment(
                deployment_id, request, model, dataset, handle
            )
            self.registry.update(
                deployment_id,
    ...
    def _prepare_model_artifact(
        ...
        if archive_path.exists() and not request.force_model:
            self.registry.update(
                deployment_id,
                stage=DeploymentStage.PREPARING_MODEL,
        ...
        self.registry.update(
            ...
            metadata={
                "model_sha": sha,
                "model_size_mb": artifact.unpacked_size_mb + artifact.archive_size_mb,
            },
        )
        return artifact
    ...
    def _prepare_dataset_artifact(
        ...
        cache_root = request.dataset_cache_dir or (
            Path.home() / ".datasets" / safe_repo / revision
        )
        cache_root.mkdir(parents=True, exist_ok=True)
        if (
            cache_root.exists()
            and any(cache_root.iterdir())
            and not request.force_dataset
        ):
            download_dir = cache_root
            self.registry.update(
                deployment_id,
        ...
        archive_dir = request.dataset_archive_dir or (
            Path.home() / ".datasets" / safe_repo
        )
        archive_dir.mkdir(parents=True, exist_ok=True)
        archive_path = archive_dir / f"{revision}.zip"
        if archive_path.exists() and not request.force_dataset:
            self.registry.update(
                deployment_id,
                stage=DeploymentStage.PREPARING_DATASET,
    ...
    def upload_dataset_artifact(
        ...
        if artifact:
            raise NotImplementedError(
                "Dataset uploads not implemented for this workflow"
            )
